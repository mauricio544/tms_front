...
private readonly router = inject(Router);
  ...
  // Vista: 'cards' (tarjetas) o 'grid' (tabla)
  ...
  originFilter: number | null = null;
  destinationFilter: number | null = null;
  ...
  // Vista de creación como ruta
  isCreateView = false;
  private syncCreateViewFromRoute() {
    const p = this.route.snapshot.routeConfig?.path || '';
    this.isCreateView = p.includes('envios/nuevo');
  }


  
  // Vista de creaci?n como ruta
  isCreateView = false;
  private syncCreateViewFromRoute() {
    const p = this.route.snapshot.routeConfig?.path || '';
    this.isCreateView = p.includes('envios/nuevo');
  ...
  // Lookup por documento (RUC/DNI)
  remitenteDocType: 'RUC' | 'DNI' = 'DNI';
  remitenteDocNumber = '';
  remitenteLookupLoading = false;
  remitenteLookupError: string | null = null;
  remitenteLookupResult: { nombre: string; documento: string } | null = null;

  destinatarioDocType: 'RUC' | 'DNI' = 'DNI';
  destinatarioDocNumber = '';
  destinatarioLookupLoading = false;
  destinatarioLookupError: string | null = null;
  destinatarioLookupResult: { nombre: string; documento: string } | null = null;

  private matchPersonaByDocumento(nro: string): Persona | undefined {
    const doc = String(nro || '').trim();
    return (this.personas || []).find(p => String((p as any).nro_documento || '').trim() === doc);
  }

  lookupRemitente() {
    this.remitenteLookupError = null;
    this.remitenteLookupResult = null;
    const tipo = this.remitenteDocType;
    const nro = (this.remitenteDocNumber || '').trim();
    if (!nro) { this.remitenteLookupError = 'Ingrese número de documento'; return; }
    this.remitenteLookupLoading = true;
    const obs = tipo === 'RUC'
      ? this.personasSrv.getDatosRUC('RUC', nro)
      : this.personasSrv.getDatosDNI('DNI', nro);
    obs.subscribe({
      next: (res: any) => {
        this.remitenteLookupLoading = false;
        const nombre = tipo === 'RUC' ? (res?.razon_social || '') : (res?.full_name || '');
        const documento = tipo === 'RUC' ? (res?.numero_documento || nro) : (res?.document_number || nro);
        this.remitenteLookupResult = { nombre: String(nombre), documento: String(documento) };
        const p = this.matchPersonaByDocumento(documento);
        if (p) { this.selectRemitente(p); }
      },
      error: () => { this.remitenteLookupLoading = false; this.remitenteLookupError = 'No se encontraron datos'; }
    });
  }
  lookupDestinatario() {
    this.destinatarioLookupError = null;
    this.destinatarioLookupResult = null;
    const tipo = this.destinatarioDocType;
    const nro = (this.destinatarioDocNumber || '').trim();
    if (!nro) { this.destinatarioLookupError = 'Ingrese número de documento'; return; }
    this.destinatarioLookupLoading = true;
    const obs = tipo === 'RUC'
      ? this.personasSrv.getDatosRUC('RUC', nro)
      : this.personasSrv.getDatosDNI('DNI', nro);
    obs.subscribe({
      next: (res: any) => {
        this.destinatarioLookupLoading = false;
        const nombre = tipo === 'RUC' ? (res?.razon_social || '') : (res?.full_name || '');
        const documento = tipo === 'RUC' ? (res?.numero_documento || nro) : (res?.document_number || nro);
        this.destinatarioLookupResult = { nombre: String(nombre), documento: String(documento) };
        const p = this.matchPersonaByDocumento(documento);
        if (p) { this.selectDestinatario(p); }
      },
      error: () => { this.destinatarioLookupLoading = false; this.destinatarioLookupError = 'No se encontraron datos'; }
    });
  }

  createRemitenteFromLookup() {
    const r = this.remitenteLookupResult; if (!r) return;
    const tipo = this.remitenteDocType;
    const payload: Partial<Persona> = tipo === 'RUC'
      ? { tipo_documento: 'RUC', nro_documento: r.documento, razon_social: r.nombre, nombre: '', apellido: '' }
      : { tipo_documento: 'DNI', nro_documento: r.documento, nombre: r.nombre, apellido: '' } as any;
    this.personasSrv.createPersona(payload).subscribe({
      next: (p: any) => { this.personas = [p as any, ...(this.personas||[])]; this.selectRemitente(p as any); },
    });
  }
  createDestinatarioFromLookup() {
    const r = this.destinatarioLookupResult; if (!r) return;
    const tipo = this.destinatarioDocType;
    const payload: Partial<Persona> = tipo === 'RUC'
      ? { tipo_documento: 'RUC', nro_documento: r.documento, razon_social: r.nombre, nombre: '', apellido: '' }
      : { tipo_documento: 'DNI', nro_documento: r.documento, nombre: r.nombre, apellido: '' } as any;
    this.personasSrv.createPersona(payload).subscribe({
      next: (p: any) => { this.personas = [p as any, ...(this.personas||[])]; this.selectDestinatario(p as any); },
    });
  }
  ...
  lookupRemitente() {
    ...
    const tipo = this.remitenteDocType;
    const nro = (this.remitenteDocNumber || '').trim();
    if (!nro) { this.remitenteLookupError = 'Ingrese n?mero de documento'; return; }
    this.remitenteLookupLoading = true;
    const obs = tipo === 'RUC'
      ? this.personasSrv.getDatosRUC('RUC', nro)
      : this.personasSrv.getDatosDNI('DNI', nro);
  ...
  lookupDestinatario() {
    ...
    const tipo = this.destinatarioDocType;
    const nro = (this.destinatarioDocNumber || '').trim();
    if (!nro) { this.destinatarioLookupError = 'Ingrese n?mero de documento'; return; }
    this.destinatarioLookupLoading = true;
    const obs = tipo === 'RUC'
      ? this.personasSrv.getDatosRUC('RUC', nro)
      : this.personasSrv.getDatosDNI('DNI', nro);
  ...
  get filteredEnvios(): Envio[] {
    ...
    return (this.lista_envios || []).filter((e: any) => {
      ...
      if (this.originFilter && Number(e?.punto_origen_id) !== Number(this.originFilter)) return false;
      if (this.destinationFilter && Number(e?.punto_destino_id) !== Number(this.destinationFilter)) return false;
      if (!term) return true;
  ...
  setView(mode: 'cards' | 'grid') {
    this.viewMode = mode;
    try { localStorage.setItem('envios.viewMode', mode); } catch {}
  }
  ...
  goToList() { this.router.navigate(['/envios']); }
  ...
  submitEnvio() {
    ...
    obs.subscribe({
      next: (res: any) => {
        ...
        this.onFilterChange();
        this.closeModal();
        this.showNotif(wasEditing ? 'Env?o actualizado' : 'Env?o creado');
      },
      error: () => {
        this.saving = false;
        this.saveError = this.editing ? 'No se pudo actualizar el env?o' : 'No se pudo crear el env?o';
  ...
  askDelete(item: Envio) {
    ...
    this.confirmMessage = `?Eliminar env?o ${this.pendingDeleteLabel}?`;
  ...
  onConfirmDelete() {
    ...
    this.enviosSrv.deleteEnvios(id).subscribe({
      next: () => {
        ...
        this.onFilterChange();
        this.onCancelDelete();
        this.showNotif('Env?o eliminado');
      },
      error: () => {
        this.saving = false;
        this.saveError = 'No se pudo eliminar el env?o';
        this.onCancelDelete();
        this.showNotif(this.saveError as string, 'error');
      }
    });
  ...
  loadEnvios() {
    ...
    this.enviosSrv.getEnvios().subscribe({
      ...
      error: () => { this.loading = false; this.error = 'No se pudieron cargar los env?os'; },
    });
  }
  ...
  ngOnInit(): void {
    this.syncCreateViewFromRoute();
this.syncCreateViewFromRoute();
    try { const saved = (localStorage.getItem('envios.viewMode') || '').toLowerCase(); if (saved === 'grid' || saved === 'cards') this.viewMode = saved as any; } catch {}
    this.loadEnvios();
    this.loadPersonas();
    this.loadPuntos();
  }
